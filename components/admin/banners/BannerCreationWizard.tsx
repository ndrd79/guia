/**
 * BannerCreationWizard - Modal wizard COMPLETO para criar banner
 * 
 * Fluxo: Página → Posição → Imagem → Configurações → Salvar
 */

import { useState, useEffect } from 'react'
import { X, ArrowLeft, ArrowRight, Home, Newspaper, Store, Calendar, Phone, Globe, ChevronRight, Image as ImageIcon, Check, Loader2, Link as LinkIcon, Settings, Save } from 'lucide-react'
...
interface BannerCreationWizardProps {
    isOpen: boolean
    onClose: () => void
    onSuccess: () => void // Chamado após salvar com sucesso
    accessToken?: string | null // Token de autenticação passado pelo componente pai
}
...
export default function BannerCreationWizard({ isOpen, onClose, onSuccess, accessToken }: BannerCreationWizardProps) {
    const [step, setStep] = useState<WizardStep>('page')
    ...
    const loadBannerCounts = async () => {
        if (!supabase) return
        try {
            const { data } = await supabase.from('banners').select('posicao')
            if (data) {
                const counts: Record<string, number> = {}
                data.forEach(b => { counts[b.posicao] = (counts[b.posicao] || 0) + 1 })
    ...
    const handleSave = async () => {
        ...
        try {
            // Usar nova API que usa cookies (não precisa de header Authorization)
            const response = await fetch('/api/admin/createBanner', {
                method: 'POST',
                headers: {
            ...
            <div className={`bg-white rounded-xl w-full max-h-[90vh] overflow-hidden shadow-2xl transition-all ${getModalWidth()}`}>
                ...
                <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-lg font-semibold text-gray-900">Novo Banner</h2>
                                ...
                                {positions.map((position) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full p-4 rounded-xl border-2 transition-all text-left flex items-center gap-4 ${isHovered ? 'border-orange-400 bg-orange-50 shadow-md' : 'border-gray-200 hover:border-indigo-400'
                                                }`}
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    {count > 0 && <span className="text-xs text-indigo-600">{count} banner(s)</span>}
                                                </div>
                                            </div>
                        ...
                        <div className="space-y-6">
                            ...
                            {formState.imagem && (
                                <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    <img src={formState.imagem} alt="Preview" className="w-24 h-16 object-cover rounded" />
                                    <div>
                                        <p className="text-sm font-medium text-gray-900">{formState.posicao}</p>
                                        <p className="text-xs text-gray-500">{formState.largura}×{formState.altura}px</p>
                                    </div>
                                </div>
                            )}
